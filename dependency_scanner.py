...
Scan + HasAllDependencies. RepointPcgReferences (the PCG-specific "search the
whole Local Library by content hash and repoint" mechanism) is NOT ported
here — it belongs with the PCG/merge-cache pull pipeline other agents are
building; this module only answers "what does this body reference" and
"which of those references are missing", independent of what "resolves" a
reference.
...
def walk_object_references(obj_type: int, body: bytes) -> Iterator[ObjectRef]:
    ...
    carry. Port of ObjectReferenceWalker.Walk. Blank Set List slots are
    skipped (see _setlist_slot_is_blank); Song refs (slot type 2) are out of
    scope, same as the C# source's own note. Any other object type yields
    nothing."""
...
def scan(resolver: Resolver, obj_type: int, body: bytes) -> List[ObjectRef]:
    ...
    lookup, not just LocalLibraryCache)."""
    return [r for r in walk_object_references(obj_type, body)
...
def has_all_dependencies(resolver: Resolver, obj_type: int, body: bytes) -> bool:
    ...
    return all(resolver(r.ref.obj_type, r.ref.bank, r.ref.number)
               for r in walk_object_references(obj_type, body))
