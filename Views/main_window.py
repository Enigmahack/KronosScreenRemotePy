...
class FrameWidget(QWidget):
    ...
    def _ed_mouse_down(self, pos: QPointF):
        ...
        if not go:
            return
        from Rendering.overlay_renderer import _SWATCH_SIZE, _SWATCH_COLS, _SWATCH_ROWS, _SLIDER_W
        rx = pos.x() - go.x()
        ry = pos.y() - go.y()
    ...
    def _ed_hover(self, pos: QPointF):
        ...
        if not go:
            return
        from Rendering.overlay_renderer import _SWATCH_SIZE, _SWATCH_COLS, _SWATCH_ROWS
        rx = pos.x() - go.x()
        ry = pos.y() - go.y()
        if 0 <= rx < _SWATCH_COLS * _SWATCH_SIZE and 0 <= ry < _SWATCH_ROWS * _SWATCH_SIZE:
            col = int(rx / _SWATCH_SIZE)
...
class MainWindow(QMainWindow):
    ...
    def _open_command_palette(self):
        from Commands.command_palette import CommandEntry, CommandPalette
        entries = [
    ...
    def _open_vu_device_picker(self):
        from Rendering.vu_meter import list_audio_devices
